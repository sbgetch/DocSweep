...
class MASWSite(BaseSite):
    ...
    SEARCH_INPUT = (By.ID, "mcpForm:attValue")

    SEARCH_BUTTON = (By.ID, "mcpForm:advancedSearchButton")
    ...
    def attach(self):
        ...
        raise RuntimeError("MASW tab is not open.")
    ...
    def execute_search(self, control_number: str):
        ...
        logger.info("MASW search completed.")

    def wait_for_search_complete(self, control_number: str, timeout=30):

        logger.info("Waiting for MASW search.")

        self.wait_for_status_update(control_number, timeout)

        logger.info("MASW search finished.")

    def wait_for_status_update(self, control_number: str, timeout: int):

        WaitHelper.until(
            self.driver,
            lambda driver: self.search_status_matches(control_number),
            timeout=timeout,
        )

    def search_status_matches(self, control_number: str) -> bool:

        try:

            text = self.driver.find_element(*self.SEARCH_STATUS).text.upper()

            return control_number.upper() in text
    ...
    def build_found_result(self) -> SearchResult:
        ...
        return SearchResult(site=Site.MASW, found=True, message=FOUND)

    def build_not_found_result(self) -> SearchResult:

        return SearchResult(site=Site.MASW, found=False, message=NOT_FOUND)
