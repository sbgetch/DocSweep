...
class PDCloudSite(BaseSite):
    ...
    ADVANCED_SEARCH_BUTTON = (By.CSS_SELECTOR, "a[aria-label*='Advanced Search']")

    SEARCH_INPUT = (By.CSS_SELECTOR, "input[aria-label=' Keyword']")

    SEARCH_BUTTON = (By.CSS_SELECTOR, "button[id$='::search']")

    RESULT_TABLE = (By.CSS_SELECTOR, "table[summary='Item Search Results']")
    ...
    def attach(self):
        ...
        raise RuntimeError("PD Cloud tab is not open.")
    ...
    def prepare_search(self):
        ...
        logger.info("Preparing Advanced Search.")

        if self.is_search_expanded():

            logger.info("Advanced Search already expanded.")

            return

        logger.info("Expanding Advanced Search.")

        button = WaitHelper.clickable(self.driver, self.ADVANCED_SEARCH_BUTTON)

        button.click()

        WaitHelper.until(self.driver, lambda driver: self.is_search_expanded())

        logger.info("Advanced Search expanded.")
    ...
    def wait_for_search_complete(self, timeout=30):

        logger.info("Waiting for PD Cloud search to complete.")
        ...
        try:
            ...
            WaitHelper.until(
                self.driver, lambda driver: self.table_is_busy(), timeout=2
            )
        ...
        WaitHelper.until(
            self.driver, lambda driver: not self.table_is_busy(), timeout=timeout
        )
        ...
        while time.time() < end_time:
            ...
            html = self.driver.find_element(*self.RESULT_TABLE).get_attribute(
                "innerHTML"
            )
        ...
        logger.info("PD Cloud search finished.")
    ...
    def table_is_busy(self):
        ...
        try:
            ...
            return self.BUSY_CLASS in table.get_attribute("class")
    ...
    def has_no_results(self) -> bool:
        ...
        elements = self.driver.find_elements(
            By.XPATH, "//*[normalize-space()='No results found.']"
        )

        if elements:

            logger.info("PD Cloud returned 'No results found.'")
    ...
    def build_found_result(self) -> SearchResult:
        ...
        return SearchResult(site=Site.PD_CLOUD, found=True, message=FOUND)

    def build_not_found_result(self) -> SearchResult:

        return SearchResult(site=Site.PD_CLOUD, found=False, message=NOT_FOUND)

    def is_search_expanded(self) -> bool:

        button = self.driver.find_element(*self.ADVANCED_SEARCH_BUTTON)
