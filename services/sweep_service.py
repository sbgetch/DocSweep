...
class SweepService:
    ...
    def sweep(self, documents):
        ...
        self.sweep_site(
            site=self.masw, site_name="MASW", attribute="masw", documents=documents
        )

        overall_elapsed = timedelta(seconds=round(time.perf_counter() - overall_start))

        logger.info(f"Overall elapsed: {overall_elapsed}")

        logger.info("Sweep complete.")

    def sweep_site(self, site, site_name: str, attribute: str, documents):
        ...
        for index, document in enumerate(documents, start=1):
            ...
            self.log_progress(site_name, index, total, document.control_number)

            try:

                result = site.search(document.control_number)

                setattr(document, attribute, result.message)
                ...
                setattr(document, attribute, ERROR)
    ...
    def log_progress(
        self, site_name: str, current: int, total: int, control_number: str
    ):

        logger.info(f"{site_name} " f"({current}/{total}): " f"{control_number}")

    def get_error_reason(self, exception: Exception) -> str:
